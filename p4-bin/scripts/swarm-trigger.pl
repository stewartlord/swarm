...
# Post to Swarm queue via wget first (run quiet on Windows as wget emits extra output)
my $util = 'wget';
if ($IS_WIN) {
    run_quiet(
        qw(wget --quiet --output-document NUL --timeout 10 --post-data),
...
if ($? > 126) {
    ...
    run(
        $IS_WIN
            ? qw(curl --silent --output NUL --max-time 10 --data)
